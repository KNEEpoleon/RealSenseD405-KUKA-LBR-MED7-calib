...
class ArucoNode(Node):
    def __init__(self):
        ...
        # Sreeharsha 19.03
        with open('/home/harry/cmu/capstone/calibration_ws/handeye_calibration_ws/src/handeye_calibration_ros2/handeye_realsense/config.yaml', 'r') as file:
        ...
        # self.get_logger().info(f"for the aruco dictionary: {ARUCO_DICT[aruco_dictionary_name]}")

        # print(f"for the aruco dictionary: {ARUCO_DICT[aruco_dictionary_name]}", flush=True)
        self.this_aruco_dictionary = cv2.aruco.getPredefinedDictionary(ARUCO_DICT[aruco_dictionary_name])
        self.get_logger().info(f"\n this aruco dictionary: {dir(self.this_aruco_dictionary)}")

        # print(f"\n\n this aruco dictionary: {self.this_aruco_dictionary}")
        self.this_aruco_parameters = cv2.aruco.DetectorParameters() # Sreeharsha
        # self.this_aruco_parameters = cv2.aruco.DetectorParameters_create()
        self.get_logger().info(f"\n this aruco params: {dir(self.this_aruco_parameters)}")

        # print(f"\n! this aruco params: {self.this_aruco_parameters}")
    ...
    def listener_callback(self, data):
        ...
        if marker_ids is not None:
            ...
            cv2.imwrite("/home/harry/cmu/capstone/calibration_ws/handeye_calibration_ws/src/handeye_calibration_ros2/handeye_realsense/resource/curr_frame_aruco_est.png", current_frame)
            # self.get_logger().info(f"\n everything SHOULD be a matrix: {corners}")
            # self.get_logger().info(f"\n everything SHOULD be a matrix: {type(self.aruco_marker_side_length)}")
            # self.get_logger().info(f"\n everything SHOULD be a matrix: {self.mtx.shape}")
            # self.get_logger().info(f"\n everything SHOULD be a matrix: {self.dst.shape}")
